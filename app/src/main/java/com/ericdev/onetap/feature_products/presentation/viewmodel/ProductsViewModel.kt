...
import com.ericdev.onetap.core.data.mapper.DataMappers.toProduct
...
class ProductsViewModel @Inject constructor(
    ...
    private fun getSampleProductList() {
        ...
        val type: Type = object : TypeToken<List<TempProductDtoResultItem?>?>() {}.type
        val list = Gson().fromJson<List<TempProductDtoResultItem>>(jsonString, type)
        _tempProductsState.value = Resource.Success(data = list)
    }

    var selectedProductOnNavigation: TempProductDtoResultItem? = null

    private var _currentImagePalette: MutableStateFlow<Palette?> = MutableStateFlow(null)
    ...
    fun resolveColorsFromUrl(url: String?) {
        viewModelScope.launch {
            ...
            if (result is SuccessResult) {
                val paletteBuilder = Palette.Builder(result.drawable.toBitmap())
                _currentImagePalette.value = paletteBuilder.generate()
            }
        }
    }
